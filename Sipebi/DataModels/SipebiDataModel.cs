...
namespace Sipebi {
  public class SipebiDataModel : ISipebiDataModel {
    ...
    /// </summary>
    private static List<EntriSipebi> entriSipebis { get; set; } = new List<EntriSipebi>();

    /// <summary>
    /// The list of formal word items loaded/prepared
    /// </summary>
    private static List<FormalWordItem> formalWords { get; set; } = new List<FormalWordItem>();

    /// <summary>
    /// The list of word items loaded/prepared
    /// </summary>
    private static List<WordItem> words { get; set; } = new List<WordItem>();

    /// <summary>
    /// The SiPEBI settings used for User-Mode SiPEBI
    /// </summary>
    private static SipebiSettings settings { get; set; } = new SipebiSettings();
    ...
    /// </summary>
    public List<WordItem> GetWords() => words;

    /// <summary>
    /// The function to get the User-Mode SiPEBI settings
    /// </summary>
    public SipebiSettings GetSettings() => settings;
    ...
    public Task InitialPreparation() 
      //Note: the initial preparation must be done in the SiPEBI data model instead of in the Sipebi Settings, for instance,
      //because the Sipebi Settings are supposed to be application independent
      //However, some diagnostic error information list, for example, can be application dependent - for instance in its folder location

      //Now, jenis kesalahan and bawaan is used only in editor mode, so when it is not editor, we should simple load all the settings from 
      // one settings file
      => Task.Run(() => {
        //Initialize Cryptography information
        ...
        if (PH.SipebiUserMode) {
          string dataFilepath = Path.Combine(baseDir, PH.SipebiDataFilename);
					string settingsFilepath = Path.Combine(baseDir, PH.SipebiSettingsFilename);
					if (!File.Exists(dataFilepath) || !File.Exists(settingsFilepath))
						return;
					words = SipebiSettings.DeserializeDataOrSettingsFile<KataSipebi>(dataFilepath).Words;
          settings.DiagnosticErrorInfos.Clear();
					settings = SipebiSettings.DeserializeDataOrSettingsFile<SipebiSettings>(settingsFilepath);
          IoC.DiagnosticErrorInformationList = settings.DiagnosticErrorInfos;

          //Now, if we are in the editor mode, the default errors can be simply configured
          //But if we are NOT in the editor mode, then all the errors are fixed
          //These errors are to be written in the SipebiSettings strictly
        } else {
          //Load default diagnostic information errors
          //All the sequence below is just to get IoC.DiagnosticErrorInformationList
    ...
    public Task PrepareWords() {
      return Task.Run(() => {
        var informalWords = words.Where(x => x.Source.StartsWith(DH.KBBI)).ToList();
        List<string> joinStrs = informalWords.OrderBy(x => x.Word.ToLower())
          .Select(x => x.Word + " -> " + x.CorrectionWord).ToList();
        createListOfKbbiInformalWords(joinStrs);
      });
		}

    private void createListOfKbbiInformalWords(List<string> joinStrs) {
      int noOfColumns = 4;
    ...
    private SipebiDiagnosticError correctWordDivisionFormalityWithMethodUser(WordDivision wordDiv, bool useCapitalWordChecking) {
      //Correct to use the formal word
      SipebiDiagnosticError diagnosticError = new SipebiDiagnosticError();
      Tuple<bool, bool, WordItem> result = wordDiv.CorrectInformalityUser(IoC.SipebiDM.GetWords(), useCapitalWordChecking);
      if (result.Item1) { //This is definitely an informal word
        diagnosticError.OriginalWordDivision = wordDiv;
        diagnosticError.ErrorCode = result.Item3.Source.StartsWith(DH.KBBI) ? 
          DH.KbbiInformalWordDiagnosticErrorCode : DH.SipebiInformalWordDiagnosticErrorCode;
        diagnosticError.OriginalElement = wordDiv.OriginalString;
        diagnosticError.CorrectedElement = wordDiv.ReconstructedString;        
      } else if (result.Item2) { //This is ambiguous word
        //Check if the latest formed string is an ambiguous word
        diagnosticError.OriginalWordDivision = wordDiv;
        diagnosticError.ErrorCode = DH.SipebiAmbiguousWordDiagnosticErrorCode;
        diagnosticError.OriginalElement = wordDiv.OriginalString;
        diagnosticError.CorrectedElement = "Saran: " + wordDiv.ReconstructedString;   
      }
    ...
    private SipebiDiagnosticError correctWordDivisionFormalityWithMethod(WordDivision wordDiv, bool useCapitalWordChecking) {
      //Correct to use the formal word
      SipebiDiagnosticError diagnosticError = new SipebiDiagnosticError();
      //InformalWordStringsKbbi is indeed identical with InformalWordsKbbi, because it is taken from it
      //It is separated from its source just for optimization purpose
    ...
    private SipebiDiagnosticError correctWordDivisionFormalityUser(WordDivision wordDiv) {
      SipebiDiagnosticError diagnosticError = new SipebiDiagnosticError();
      if ((wordDiv.WordPositionInSentence & WordPositionInSentence.FirstWord) != 0) {
        ...
        if (!startedWithCapital) { //false first word case
          ...
          diagnosticError = correctWordDivisionFormalityWithMethodUser(wordDiv, useCapitalWordChecking: true);
      }
      ...
      if (!diagnosticError.HasError) //if does not have error the first time, try the second time
        diagnosticError = correctWordDivisionFormalityWithMethodUser(wordDiv, useCapitalWordChecking: false);
    ...
    public SipebiDiagnosticReport CorrectTextUser(string originalText) {
      ...
      foreach (var paragraph in paragraphs) {
        ...
        int currentParLength = WordDivision.ProcessWordDivisionStream(wordDivs, paragraphNo, charPositionOffset,
          null, null, null); //now, for user case, instead of having some list of words, all the word lists are set to null
        ...
        for (int i = 0; i < wordDivs.Count; ++i) {
          ...
          //Correct the word division formality first, for now, as of prototype 2, 
          //word informality is the only correction to be done
          diagnosticError = correctWordDivisionFormalityUser(wordDiv); 
          ...
          currentCorrectedString = diagnosticError.ErrorCode == DH.SipebiAmbiguousWordDiagnosticErrorCode ?
            wordDiv.OriginalString : wordDiv.ReconstructedString; //if it is ambiguous, do not correct it
