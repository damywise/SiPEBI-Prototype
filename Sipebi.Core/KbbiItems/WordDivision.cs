...
namespace KBBI.Core {
  ...
  public class WordDivision {
    ...
    //First bool: it is informal word
    //Second bool: it is ambiguous word
		public Tuple<bool, bool, WordItem> CorrectInformalityUser(List<WordItem> items, bool useCapitalWordChecking = false) {
      if (items == null || items.Count <= 0)
        return new Tuple<bool, bool, WordItem>(false, false, null);
      //Check for direct, non-capitalized, translation
      var wItem = items.FirstOrDefault(x => x.Word == CleanWordString);
      if (wItem != null) { //then the process begins
        List<string> properties = wItem.GetWordProperties();
        if (properties.Contains(DH.takbaku)) {
          CleanWordString = wItem.CorrectionWord; //we return the correction but it is not necessarily applied yet
          return properties.Contains(DH.takbaku_ambigu) ?
            new Tuple<bool, bool, WordItem>(false, true, wItem) :
            new Tuple<bool, bool, WordItem>(true, false, wItem);
        }
        //if the word does not contain takbaku property, then we may continue
      }

      //At this point, if capital word checking is not used, then simply returns the word
      if (!useCapitalWordChecking) 
        return new Tuple<bool, bool, WordItem>(false, false, null);

      //get lower-cased version (of supposedly capitalized word)
      wItem = items.FirstOrDefault(x => x.Word == CleanWordString.ToLower()); 
      if (wItem != null) {
        List<string> properties = wItem.GetWordProperties();
        if (properties.Contains(DH.takbaku)) {
          CleanWordString = wItem.CorrectionWord;
