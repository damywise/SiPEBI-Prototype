...
namespace Sipebi.Core {
    ...
    public static OptionListViewModel ViewInstance { get; } = new OptionListViewModel();
		...
		private List<OptionListItemViewModel> editorList = new List<OptionListItemViewModel> {
		...
		private List<OptionListItemViewModel> userList = new List<OptionListItemViewModel> {
			new OptionListItemViewModel {
				OptionText = "Benahi Teks",
				OptionIcon = "\uf016", //TODO not a good practice, but leave it for now
				OptionToolTip = "Halaman penyuntingan ejaan",
		...
		public OptionListViewModel() { Items = PH.SipebiUserMode ? userList : editorList; }
